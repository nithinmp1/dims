...
<div class="row">
    ...
    foreach ($types as $table => $type) {
        $allow = $this->crud_model->accessCheck(['dashboard',$table.'_report']);
        if(!$allow){
            unset($types[$table]);
        }
    }
    ...
    <div class="row" >
        ...
        <?php foreach ($types as $table => $type) { ?>
        <div class="col-sm-6 reportControllers" <?=($table == array_key_first($types))?'':'style = "display: none;"'?> data-type="<?=$table?>">
    ...
    <?php
    foreach ($types as $table => $type) { ?>
        <h3 class="cardHead" data-table="<?=$table?>" style="<?=($table == array_key_first($types))?'':'display :none'?>"><?=get_phrase($table)?> Report</h3>
    <?php
        foreach($type as $type) {
    ?>
        <div class="row card" style="<?=(($table == array_key_first($types)) && ($type == $types[array_key_first($types)][0])) ?'':'display :none'?>" data-type="<?=$type?>" data-table="<?=$table?>">
            <?php 
