        ...
        <ul class="nav nav-tabs bordered">
            <li class="active">
                <a href="#home" data-toggle="tab">
                    ...
                    if((isset($param1) === true) && $param1 != ''){
                    echo $param1." ";
                    }
                    echo get_phrase('all_students'); ?></span>
                </a>
            </li>
        </ul>
                ...
                <table class="table table-bordered datatable" id="table_export">
                    <thead>
                        <tr>
                            ...
                            <th><div><?php echo get_phrase('Ref-ID'); ?></div></th>
                            <th width="80"><div><?php echo get_phrase('photo'); ?></div></th>
                            <th ><div><?php echo get_phrase('stream'); ?></div></th>
                            <th><div><?php echo get_phrase('name'); ?></div></th>
                            <th><div><?php echo get_phrase('phone'); ?></div></th>
                            <th><div><?php echo get_phrase('gender'); ?></div></th>
                            <th><div><?php echo get_phrase('lic/ler No'); ?></div></th>
                            <th><div><?php echo get_phrase('application No'); ?></div></th>
                            <th><div><?php echo get_phrase('class status'); ?></div></th>
                    ...
                    <tbody>
                        ...
                        if((isset($stream_id) === true) && $stream_id != ''){
                        ...
                        foreach ($students as $keyRow => $row):
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php
                                    $student_code = $this->db->get_where('student', array(
                                        'student_id' => $row['student_id']
                                    ))->row()->student_code;
                                    ?>
                                    <a target="_blank" href="<?=base_url('index.php?student/profile/'.$student_code)?>"><?=$student_code?></a>
                                </td>
                                <td>
                                    <img src="<?php echo $this->crud_model->get_image_url('student', $row['student_id']); ?>" class="img-circle" width="30" />
                                </td>
                                <td>
                                ...
                                <td>
                                    <?php
                                    $licence_number = $this->db->get_where('student', array(
                                        'student_id' => $row['student_id']
                                    ))->row()->licence_number;
                                    ...
                                    ?>
                                    <?php
                                    $learners_number = $this->db->get_where('student', array(
                                        'student_id' => $row['student_id']
                                    ))->row()->learners_number;

                                    if($learners_number != ''){
                                        echo $learners_number.' ||';
                                    }
                                    ?>
                                    <?php
                                    echo $this->db->get_where('student', array(
                                        'student_id' => $row['student_id']
                                    ))->row()->driving_licence_number;
                                    ?>
                                </td>
                                <td>
                                    <?php
                                ...
                                <td>
                                    <?php
                                    $enroll = $this->db->get_where('enroll',['student_id' => $row['student_id']])->result_array();
                                    foreach($enroll as $enrollK => $enrollV){
                                        $class_status = ($enrollV['class_count'] - $enrollV['class_count_status']).' Class Remaining';
                                        $class = $this->db->get_where('class',['class_id' => $enrollV['class_id']])->row_array(); 
                                        echo $class['name'].'- '.$class_status."<br>";
                                    }
                                    ?>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu dropdown-default pull-right" role="menu">
                                            ...
                                            <!-- VIEWING LINK -->
                                            <?php
                                            $invoices = $this->db->get_where('invoice', array('student_id' => $row["student_id"]))->row_array();
                                            if (!empty($invoices)) { ?>
                                            <li>
                                                <a href="#" onclick="showAjaxModal('<?php echo base_url(); ?>index.php?modal/popup/modal_view_student_invoice/<?php echo $row['student_id']; ?>');">
