...
class Mobile extends CI_Controller
{
    ...
    function get_instructors() {
        ...
        foreach ($instructors as $row) {
            ...
            $data['phone']      =   $row['phone'];
            $data['email']      =   $row['email'];
            $data['image_url']  =   $this->crud_model->get_image_url( 'instructor' , $row['instructor_id'] );
    ...
    function get_instructor_profile() {
        ...
        $response       =   $this->db->get_where('instructor' , array('instructor_id' => $instructor_id))->row();
        echo json_encode($response);
    ...
    function get_subject_of_class() {
        ...
        foreach ($subjects as $row) {
            ...
            $instructor_query              =   $this->db->get_where('instructor' , array('instructor_id' =>  $row['instructor_id']));
            if ( $instructor_query->num_rows() > 0) {
                $instructor_query_row      =   $instructor_query->row();
                $data['instructor_name']   =   $instructor_query_row->name;
            }
            else {
    ...
    function get_total_summary() {
        ...
        $response['total_instructor']      = $this->db->count_all('instructor');
    ...
    function login() {
        ...
        $query = $this->db->get_where('instructor', array('email' => $email , 'password' => $password));
        if ($query->num_rows() > 0) {
            $row = $query->row();

            $authentication_key         =   md5( rand(10000, 1000000));
            $response['status']         =   'success';
            $response['login_type']     =   'instructor';
            $response['login_user_id']  =   $row->instructor_id;
            $response['name']           =   $row->name;
            $response['authentication_key']=$authentication_key;

            // update the new authentication key into user table
            $this->db->where('instructor_id' , $row->instructor_id);
            $this->db->update('instructor' , array('authentication_key' => $authentication_key));
            echo json_encode($response);
            return;
